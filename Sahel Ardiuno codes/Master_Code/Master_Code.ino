...
void loop() {
  ...
  // turns on the LED every three button pushes by checking the modulo of the button push counter
  if (buttonPushCounter % 3 == 0) {
    /*buttonState = digitalRead(buttonPin);
    unsigned long CurrentTime = millis();
    unsigned long ElapsedTime = CurrentTime - StartTime;
    if (buttonpin==1 && ElapsedTime==10000){*/
    //digitalWrite(ledPin, HIGH);

    //CODE INITIALIZES

    //assuming HC05 is already connected to the app
    //sending character to initiliaze app functions
    if (Serial.available())
    {   
      Serial.write('A');
    } 
    ...
    //OV7670 t SDCard in .bmp format
      ...
      delay(2000);
      QVGA_Image("8.bmp");
      //Serial.print("Images captured sucessfully");
      while(1);
  } 
  }
}
